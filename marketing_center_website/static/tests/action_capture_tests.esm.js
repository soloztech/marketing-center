...
QUnit.test(
    "preserves the strict native WhatsApp fallback when tracking is unavailable",
    (assert) => {
        function eventFor(href) {
            const link = document.createElement("a");
            link.setAttribute("data-marketing-whatsapp-action", ACTION_REF);
            link.setAttribute("href", href);
            return {composedPath: () => [link]};
        }
        const origin = "https://www.example.test";
        assert.strictEqual(
            whatsappFallbackPath(
                eventFor("https://wa.me/5519999999999?text=Ola"),
                origin
            ),
            "https://wa.me/5519999999999?text=Ola"
        );
        for (const href of [
            "http://wa.me/5519999999999",
            "https://wa.me.evil.test/5519999999999",
            "https://wa.me/5519999999999?redirect=https://evil.test",
            "https://wa.me/5519999999999#secret",
            "https://user@wa.me/5519999999999",
        ]) {
            assert.strictEqual(whatsappFallbackPath(eventFor(href), origin), "", href);
        }
    }
);

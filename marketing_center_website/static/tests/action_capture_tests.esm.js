...
QUnit.test(
  "adds only opaque references to the exact native form route",
  (assert) => {
...
QUnit.test(
  "passes the native body and promise through by identity",
  async (assert) => {
    assert.expect(5);
    ...
    assert.strictEqual(
      returned,
      nativePromise,
      "the native promise is unchanged"
    );
    assert.strictEqual(
      receivedBody,
      opaqueBody,
      "the native body is unchanged"
    );
...
QUnit.test("does not consume a claim for unrelated legacy AJAX", (assert) => {
  ...
  assert.notOk(
    nativeFormTarget("/web/dataset/call_kw", "https://www.example.test")
  );
...
QUnit.test("requires a visible trusted user activation", (assert) => {
  ...
  assert.notOk(
    trustedHumanActivation({ ...event, isTrusted: false }, visible, active)
  );
  assert.notOk(
    trustedHumanActivation(event, { visibilityState: "hidden" }, active)
  );
  assert.notOk(
  ...
  assert.notOk(
    trustedHumanActivation({ ...event, ctrlKey: true }, visible, active)
  );
});

QUnit.test(
  "reads only the explicit technical marker from the event path",
  (assert) => {
...
QUnit.test(
  ...
  (assert) => {
    ...
    assert.ok(
      nativeTrackedLinkActivation(nativeEvent, "https://www.example.test")
    );
    assert.strictEqual(
...
QUnit.test(
  "keeps WhatsApp claims and redirects opaque and bounded",
  (assert) => {
    ...
    assert.ok(
      validRedirectPath(`/marketing/website-action/go/${"A".repeat(43)}`)
    );
    assert.notOk(validRedirectPath("https://wa.me/5519999999999"));
    assert.notOk(
      validRedirectPath(
        `/marketing/website-action/go/${"A".repeat(43)}?next=evil`
      )
    );
  }
);
...
QUnit.test(
  "keeps a safe ordinary link as the WhatsApp fail-open path",
  (assert) => {
    ...
    assert.strictEqual(
      whatsappFallbackPath(
        activation("https://www.example.test/support"),
        origin
      ),
      "/support"
    );

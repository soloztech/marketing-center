...
Ads Search pagination over the official REST v25 endpoints. URLs are fixed,
redirects are disabled, responses are streamed under byte limits and quota,
authentication, permission and transient failures use a sanitized error taxonomy.
No campaign, lead, attribution or other business-domain model lives here.

Only Odoo Settings administrators (``base.group_system``) receive model ACLs, and
the record rule limits identities to active companies. The technical addon has no
menus or views; provider addons own their configuration experience.

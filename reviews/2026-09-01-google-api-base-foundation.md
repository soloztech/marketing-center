...
Data: 2026-09-01 Escopo: somente `integration-core/google_api_base`; nenhuma instalação,
alteração de banco, deploy ou mudança em Marketing Center/Contact Center/scripts.

## Resultado

Foi criado o addon técnico `google_api_base` `16.0.1.0.0`, dependente apenas de `base` e
`requests`. Ele contém:

- `google.api.identity` multiempresa, com `public_ref` imutável e revisão monotônica;
- referências environment/file para OAuth client ID, OAuth client secret, refresh token
  e developer token; nenhum valor de credencial é persistido;
...
O transporte não depende de `queue_job`: o futuro addon consumidor deve chamar a fachada
dentro de jobs OCA. Assim o core continua neutro e não ganha fila ou regra de
...
1. **Contexto oculto de exceção sensível.** `raise ... from None` ainda preservava a
   exceção original em `__context__`. Os limites de rede, JSON, encoding e arquivo agora
   saem do bloco `except` antes de levantar o erro sanitizado; testes verificam
...
3. **Fence opcional.** `_resolve_runtime()` agora exige uma revisão inteira positiva;
   ...
   das credenciais.
4. **OAuth antes de validar conta.** A fachada normaliza e valida o customer ID antes de
   adquirir token, então entrada inválida não provoca IO.
...
- A ausência de `TransactionCase` concorrente não contradiz o locking implementado, mas
  continua como gate de integração para comprovar a serialização real do `FOR UPDATE` no
  PostgreSQL/Odoo.
- O read timeout de `requests` não é um deadline de socket contra slow-drip. URL fixa,
  limite de bytes e orçamento agregado reduzem o risco, mas um worker/SDK isolado poderá
  ser reavaliado se o volume real justificar.
...
- `compileall`, AST do manifest, XML, CSV, Black, isort, `git diff --check` e flake8 com
  `max-complexity=16`: verdes;

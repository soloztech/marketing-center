...
Data: 2026-09-01  
Escopo: candidato local de `marketing_center_base`; sem implantação.

## Decisão

O corte adiciona modelos versionados e um ledger imutável de cálculo:

- `marketing.attribution.model` congela estratégia, janela, policy e bases de
...
- `marketing.attribution.candidate` preserva toda evidência candidata, inclusive a
  que foi excluída;
- `marketing.attribution.contribution` contém somente touchpoints efetivos que
  receberam peso.

Uma nova execução cria novo `calculation_ref` e não substitui a anterior. Replay com
o mesmo input devolve o mesmo resultado; reutilizar a referência com input diferente
...
`correlation_only` é persistido no ledger de candidatos para explicar a cobertura,
mas nunca pode ser configurado como base creditável. Relações M:N existentes entre
conversa, pessoa, lead e touchpoint, por si sós, portanto não geram contribuição.
...
ou contrafactual. `platform_reported` vive em estratégia e interpretação separadas e
não pode ser misturado ao crédito da jornada.

## Trust boundary

O serviço exige uma capability Python interna e um `producer_key` estável. Não é uma
API RPC/UI. Cada `evidence_ref` deve ser uma referência imutável e namespaced emitida
pelo produtor, por exemplo `website.session_event:<uuid>`; texto digitado na UI não é
evidência aceitável.
...
pode receber crédito até existir um ledger próprio de decisão, com autoridade,
autor, ocorrência e revisão imutáveis. Esse produtor é um gate futuro, não uma
causalidade presumida agora.

Para `platform_reported`, cada candidato traz peso reportado. Peso zero continua no
ledger, com estado `eligible_not_selected`, e não cria contribuição. Os pesos
positivos precisam fechar exatamente `1_000_000`.
...
- create/write/unlink direto em qualquer ledger novo é negado, inclusive com
  `sudo()` sem o token interno;
- ACL e record rules mantêm o detalhe de cálculo restrito ao administrador e às
  empresas ativas;

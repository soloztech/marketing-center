#!/usr/bin/env python3
"""Run acquisition simplification regression suites in a task-owned local PostgreSQL cluster.
...
def main():
    ...
    parser.add_argument("--http-port", type=int, default=18219)
    ...
    if state.name != "20260919-marketing-simplification-lab":
        parser.error("state must be the dedicated simplification directory")
    state.mkdir(parents=True, exist_ok=True, mode=0o700)
    marker = state / "local-cluster-owner"
    expected = "marketing-simplification-20260919\n"
    ...
    env = {
        ...
        if not key.startswith(("PG", "ODOO_", "META_", "GOOGLE_"))
        and key.lower() not in ("http_proxy", "https_proxy", "all_proxy")
    }
    if (state / "bin/google-chrome").exists():
        env["PATH"] = str(state / "bin") + os.pathsep + env.get("PATH", "")
    env["ODOO_META_API_SECRET_DIR"] = str(state / "no-secrets")
    env["ODOO_GOOGLE_API_SECRET_DIR"] = str(state / "no-secrets")
    env.update(
        PGOPTIONS="-c jit=off",
    ...
    if args.action == "init":
        ...
        run(
            [
                ...
                state / "postgres.log",
                "-o",
                f"-k {socket} -p 55499 -c listen_addresses='' -c jit=off",
                "-w",
                "start",
            ]
        )
    ...
    db = "mc_simplify_20260919_" + args.suite
    command = [
